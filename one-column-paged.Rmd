...
## Research Experience {.nohead .research_exp .set-width}
```{r}
cv_entries %>% 
  filter(type == 'research_exp') %>% 
...
## Conference Presentations {.nohead .talks .set-width}
```{r}
cv_entries %>% 
...
## Non-referred Conference Presentations {.nohead .posters .set-width}
```{r}
cv_entries %>% 
  filter(type == 'nconference') %>%
  mutate(what = str_c("", what, ", ", where)) %>% 
  select(year_begin, what) %>% 
...
## Invited Talks {.media .nohead .set-width}
```{r}
cv_entries %>% 
  filter(type == 'talk') %>%
  mutate(what = str_c("", what, ", ", where)) %>% 
...
## Research experience {.work .nohead .vertical-top .set-width}
```{r}
cv_entries %>% 
  filter(type == 'research', is.na(one_column_paged)) %>% 
...
## Administrative Experience {.prof-dev .nohead .set-width}
```{r}
cv_entries %>% 
  filter(type == 'admin') %>% 
  mutate(what = str_c(what, ", ", where)) %>% 
